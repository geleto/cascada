...
class Parser extends Obj {
  ...
  parseSet(tagName = 'set', varType = 'assignment', endTagName = 'endset') {
    const tag = this.peekToken();
    if (!this.skipSymbol(tagName)) {
  ...
  _parseVarLikeDeclaration(tagName, varType, endTagName, errorPrefix) {
    const tag = this.peekToken();
    if (!this.skipSymbol(tagName)) {
    ...
    if (!this.skipValue(lexer.TOKEN_OPERATOR, '=')) {
      ...
      if (this.skip(lexer.TOKEN_COLON)) {
        this.fail(`${errorPrefix}: focus directives are not supported`, tag.lineno, tag.colno);
      }

      if (!this.skip(lexer.TOKEN_BLOCK_END)) {
        this.fail(`${errorPrefix}: expected = or block end in ${tagName} tag`,
          tag.lineno,
          tag.colno);
      } else {
        node.body = new nodes.Capture(
  ...
  parseChannelDeclaration(isShared = false) {
    ...
    if (channelType === 'sink' || channelType === 'sequence') {
      ...
      if (this.skipValue(lexer.TOKEN_OPERATOR, '=')) {
        if (channelType === 'var' || (!isShared && (channelType === 'data' || channelType === 'text'))) {
          initializer = this.parseExpression();
        } else {
