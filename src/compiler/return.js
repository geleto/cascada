...
class CompileReturn {
  ...
  emitIsUnsetCall(node) {
    const compiler = this.compiler;
    this._validateIsUnsetCall(node);
    if (!node._analysis.visibleDeclarations?.get(RETURN_CHAIN_NAME)) {
      compiler.fail(
  ...
  getSequentialLoopAdvanceCheckChain({ sequentialLoopBody, whileConditionNode, bodyReturnStateChains }) {
    return sequentialLoopBody &&
      !whileConditionNode &&
      this.hasReturnStateObservation(bodyReturnStateChains)
      ? RETURN_CHAIN_NAME
      : null;
  }
}
