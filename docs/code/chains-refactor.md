...
- Runtime boundary helpers now live in `src/runtime/async-boundaries.js`:
  ...
  - `runRenderBoundary(...)`
  - `runValueBoundary(...)`
- Compiler boundary helpers now live in `src/compiler/async-boundaries.js`:
  - `compileControlFlowBoundary(...)`
  - `compileRenderBoundary(...)`
  - `compileTextBoundary(...)`
  - `compileCaptureBoundary(...)`
- `runtime.js`, `buffer.js`, and `emit.js` now delegate to those focused boundary modules instead of owning the full implementations directly.
- `compileInclude`, `compileExtends`, `compileImport`, and `compileFromImport` now use the chains-refactor model: control-flow async stays inside `runControlFlowBoundary`, while command emission is synchronous.
- Root/value work such as template output, `set`, `do`, `return`, script `var`, and var-chain initializers is emitted synchronously as commands whose arguments may be promises.
- Sequential and bounded loops no longer use `waitAllClosures(1)` as an iteration-completion fallback.
- Sequential and bounded loop completion is driven by the per-iteration `__waited__` chain, as described in [waited-loops.md](C:\Projects\cascada\docs\code\waited-loops.md).
- Async control-flow inside waited-loop bodies now uses a child-local waited chain, and the parent loop tracks that control-flow block as one waited unit.
...
- Macro/caller analysis and compilation now live in `macro.js`, which owns:
  - `analyzeMacro` / `compileMacro` / `_compileMacro`
  - `analyzeCaller` / `compileCaller`
  - caller scheduling helpers and caller-chain filtering
- Async caller-capable macros now use:
  ...
  - a macro-local `__caller__` timing chain to track per-invocation buffer scheduling completion

Still transitional:
- `Caller` bodies now use a local `__waited__` chain so command-only child boundaries (for example async `do` statements with sequential side effects) contribute a structural completion signal before caller finalization snapshots return/text state.
...
```js
// set x = fetchUser()  — x is a VarChain
let t1 = fetchUser(); // may return a promise
parentBuffer.add(new VarCommand({ chainName: 'x', args: [t1], pos }), 'x');

// {{ asyncExpr }}
let t2 = computeExpr(); // may return a promise
parentBuffer.add(new TextCommand({ chainName: '__text__', args: [t2], pos }), '__text__');

// data result
// result.items.push(asyncValue)
let t3 = fetchItem(); // may return a promise
parentBuffer.add(new DataCommand({ chainName: 'result', command: 'push', args: [['items'], t3], pos }), 'result');
```

All of these are added synchronously. `resolveCommandArgumentsForApply` in the chain awaits the promise when the command is applied.
...
When a control-flow value is poison or its promise rejects, the branches do not execute. Instead, all chains that would have been written by those branches receive `addCommand(new ErrorCommand(...))`. This logic already exists in the current `compileIf` and related methods — it just currently lives inside larger `asyncBlock` wrappers that are being removed.
...
async (childBuf) => {
  ...
  if (isPoison(cond)) {
    childBuf.addCommand(new ErrorCommand(cond.errors), '__text__');
    childBuf.addCommand(new ErrorCommand(cond.errors), 'data');
    return; // helper's finally calls finish
  }
  ...
  if (cond && typeof cond.then === 'function') {
    try {
      ...
      childBuf.addCommand(new ErrorCommand(errors), '__text__');
      childBuf.addCommand(new ErrorCommand(errors), 'data');
      return;
    }
    if (isPoison(cond)) {
      childBuf.addCommand(new ErrorCommand(cond.errors), '__text__');
  ...
  // finish called by helper's finally
}
```
...
- the working solution is for those control-flow child buffers to own their own waited chain inside waited-loop scope

```js
// runtime.js
function runControlFlowBoundary(parentBuffer, usedChains, frame, context, asyncFn, enableWaitApplied = false) {
  const childBuf = new CommandBuffer(context, null, frame, enableWaitApplied);
  for (const ch of usedChains) {
...
- The child buffer is linked to all parent chains **synchronously** before the async fn starts.
- The main code flow **does not wait** — it continues adding commands to `parentBuffer` immediately after this call.
- `finish` is called in `.finally()` so it always fires even if there is a compiler bug in the async fn.
...
}, runtime, frame, { usedChains: ['__text__', 'data'] }, parentBuffer, true, cb);
...
parentBuffer.add(new TextCommand({ chainName: '__text__', args: [t1], pos }), '__text__');
```
...
- instead, the control-flow child buffer owns its own waited chain, and the parent waits on that child waited chain's completion

Why:
- the iteration `__waited__` chain is intentionally flat
...
- nested async control-flow inside a waited iteration is tracked as one parent-visible waited unit via the child control-flow waited-chain completion promise
...
- **Root buffer** (`output`): `finish()` is called immediately after all top-level statements are compiled and executed — no waiting.
- **Control-flow child buffers**: `finish()` is called by the helper's `.finally()` when the async fn completes.
- **Inline sync paths**: there is no child buffer; commands go directly into the parent, so there is nothing to finish.

The `finished` flag means "no more entries will be added to this buffer's arrays." The BufferIterator will still process all existing entries (including child buffers that are still running) before declaring a chain's stream exhausted.
...
| `asyncBlockBegin` / `asyncBlockEnd` in `emit.js` | **Phase 1** — call sites removed; the functions may remain until Phase 2 |
| `asyncBlockValue` in `emit.js` | **Phase 1** — value assignment emits no async block; result may be a promise, that is fine |
| `asyncBlockRender` in `emit.js` | **Phase 1** — replaced by `compileRenderBoundary` / `runRenderBoundary` where template execution needs an isolated render boundary |
| `asyncAddToBuffer` / `asyncAddToBufferScoped` in `buffer.js` | **Phase 1** — commands added synchronously; these helpers are no longer emitted |
| `asyncBlock` in `emit.js` (the simple wrapper) | **Phase 1** — call sites removed during migration |
| `getAsyncBlockArgs` `asyncMeta` wrapper object | **Phase 1** — `usedChains` array passed directly to `runControlFlowBoundary` via `getLinkedChainsArg` (extracted); `getAsyncBlockArgs` retained for remaining `asyncBlock` call sites |
...
| `BufferIterator` per chain | Unchanged |
...
| `CompileAnalysis` and `_analysis` metadata | Unchanged — `node.isAsync`, `usedChains`, `declaredChains` still drive decisions |
| `propagateIsAsync` | Unchanged — `node.isAsync` is the gate for whether a control-flow site needs a child buffer |
| `__waited__` / `waitApplied` / `enableWaitApplied` | Concept unchanged — but the implementation now relies on waited-chain `finalSnapshot()` rather than closure-counting fallback for sequential / bounded loop completion |
...
| `chain._recordError` / `ErrorCommand` enqueue paths | Unchanged |
| `finish` | Unchanged — called sooner and more directly |
...
## `usedChains` for Child Buffer Linking

`usedChains` is still computed from `node._analysis.usedChains` (filtered to exclude locally-declared chains, per `getAsyncBlockArgs` logic). It is passed directly to `runControlFlowBoundary` so the helper knows which parent chain arrays to insert the child buffer into.
...
function runControlFlowBoundary(astate, parentBuffer, usedChains, f, context, cb, asyncFn, enableWaitApplied = false) {
  void context;
  const asyncMeta = { usedChains: usedChains || null };
...
- Creates the child `CommandBuffer` and links it to parent chains via `usedChains`.
- Tracks closure counts so parent can `waitAllClosures` if anything still uses it.
- Calls `finish` in cleanup.

Nothing else changes in the runtime during Phase 1.

### New compiler-side helpers added in Phase 1

#### `emit.getLinkedChainsArg(node, frame)`

Extracted from `getAsyncBlockArgs`. Returns the JSON array string for the chains to link — `'["__text__","data"]'` or `'null'`. Filters `node._analysis.usedChains` the same way as `getAsyncBlockArgs` (excludes locally-declared chains and `__return__`; includes `currentWaitedChainName` when inside a limited-loop). `getAsyncBlockArgs` now delegates to this.

Used by `compileControlFlowBoundary` so `runControlFlowBoundary(astate, buf, <chainsArg>, ...)` gets the right array without wrapping it in `{ usedChains: ... }`.

#### `boundaries.compileControlFlowBoundary(node, frame, emitFunc)`

Compiler-side analog of `asyncBufferNode` for control-flow nodes. For async nodes:
1. Emits `runtime.runControlFlowBoundary(astate, ${parentBuf}, ${linkedChains}, frame, context, cb, async (astate, frame, currentBuffer) => {`
...
runtime.runControlFlowBoundary(astate, currentBuffer, ["__text__","data"], frame, context, cb,
  async (astate, frame, currentBuffer) => {
    ...
    // finish handled by astate.asyncBlock cleanup
  },
false);
...
currentBuffer.add(new TextCommand({ chainName: '__text__', args: [t1], pos }), '__text__');
```
...
As each loop `compileXXX` method is migrated, this line is removed. Sequential ordering is maintained by the order in which commands and child buffers are added to the parent buffer — but for sequential and bounded loops the actual iteration completion signal is the iteration child buffer's `__waited__` chain `finalSnapshot()`, not generic closure counting.
...
- Only add a new helper in `async-boundaries.js` if the call site has real semantics that `compileControlFlowBoundary(...)` does not cover cleanly.
...
2. ? **`compileAsyncVarSet` step 3 / `compileChainDeclaration`** — `asyncAddValueToBuffer` calls inlined; VarCommands now added synchronously via `currentBuffer.add(new VarCommand(...))` (Tier 1)
...
4. ? **`compileReturn`** — `waitAllClosures` removed; return value added as `VarCommand` to return chain synchronously (completed in prior chat)
5. ? **`compileIf`** — `asyncBufferNode` + two inner `asyncBlock` branch wrappers replaced by `compileControlFlowBoundary`; branches now compile synchronously inside the outer async fn with explicit `frame.push()`/`frame.pop()` (Tier 3). Adds `runtime.runControlFlowBoundary`, `boundaries.compileControlFlowBoundary`, `emit.getLinkedChainsArg`.
6. ? **`compileSwitch`** — same pattern as `compileIf`: `asyncBufferNode` + per-case `asyncBlock` wrappers replaced by `compileControlFlowBoundary` with case bodies compiled synchronously. `branchChains` collection simplified: reads `c.body._analysis.usedChains` directly (available before compilation, no need to collect inside async block callbacks) (Tier 3)
...
8. ? **`compileFor` (sequential / `each`)** — completed. `waitAllClosures(1)` was removed from sequential loop-body completion, but the final implementation required more than simply deleting the line:
   ...
   - nested async control-flow blocks inside waited-loop bodies now add one explicit parent waited unit via their child control-flow waited-chain completion promise
   - compiler/codegen tests were updated from the old "closure anchor" model to the new waited-unit model
9. ? **Remove `WaitResolveCommand` for `var x = expr` and `set_path` in limited loops** — `emitOwnWaitedConcurrencyResolve` removed from `compileAsyncVarSet`. The VarCommand's promise arg is already awaited by the buffer iterator as it processes the iteration buffer's var chain — the `WaitResolveCommand` in `__waited__N` was doubly tracking the same promise. Only codegen tests needed updating; no runtime regressions.
...
15. ? **Introduce a dedicated structural completion signal for text/composition boundaries**.
   ...
   - `compileInclude` now uses the composed child text chain's `finalSnapshot()` as the boundary's structural completion signal.
...
16. ? **Pre-create the real caller invocation buffer for deferred `caller()` output paths**.
   - Implemented:
     - caller invocation owns `__caller__` timing completely: only caller invocation code may add `WaitResolveCommand` entries to the macro-local `__caller__` chain
...
18. ? **Migrate macro/caller boundaries onto the structural composition model**.
   - The caller structural-attachment model is implemented and macro/caller analysis and compilation live in `macro.js`.
...
19. ? **Introduce a dedicated structural block-invocation emitter/helper before removing `asyncAddToBuffer` from block calls**.
   - Implemented:
     - async block invocation now uses a dedicated `asyncAddBlockInvocationToBuffer(...)` helper in `buffer.js`
...
21. [IN PROGRESS] **Finish removing legacy generic expression-compilation async wrappers**.
   - The plan now lives in [expression-chains.md](c:\Projects\cascada\docs\code\expression-chains.md).
...
22. ? **Replace remaining `asyncBlockRender(...)` content-render wrappers**.
   - Implemented:
     ...
     - `emit.compileRenderBoundary(...)` now lowers async render boundaries through a dedicated runtime `runRenderBoundary(...)` helper that creates an isolated unlinked child buffer, snapshots it, and cleans it up
...
24. ? **Replace remaining `asyncAddBlockInvocationToBuffer(...)` block/super/inheritance wrappers with a clearer structural boundary**.
   ...
   - Important implementation detail:
     ...
     - but the invoked block function still receives the enclosing parent buffer for visible-chain linking and composition semantics
...
27. ? **Replace the last legacy `astate.asyncBlock(... createOutputBuffer ...)` path with an explicit structural text-output boundary helper**.
   - The remaining nonconforming buffer-creation path was `asyncAddStructuralTextOutput(...)` in `buffer.js`.
   - That mutating-expression output case now lowers through `compileTextBoundary(...)` in `async-boundaries.js`.
...
28. ? **Audit remaining non-expression async-block / closure-counting paths**.
   ...
   - Current suspects:
     ...
     - root finalization
     - any remaining non-expression helper that can still add commands after `finish()`

   Completed audit result:
   - Remaining non-expression closure-counting waits are now concentrated in:
     - `macro.js` async macro finalization (`astate.waitAllClosures()`)
   ...
   - Generic expression wrappers still exist in `emit.js` / `compiler-base.js` (`asyncBlockValue(...)` and related callers), but those are explicitly out of scope for this step and are not the current blocker for macro/root cleanup.
...
30. ? **Remove remaining `waitAllClosures()` from `compileMacro` / `compileRoot`**.
   - `compileMacro` is now complete:
     ...
     - `Caller` bodies use a local `__waited__` chain so command-only child boundaries can finalize structurally without falling back to closure counting
...
31. ? **Retire remaining `asyncBufferNode(...)` call sites one by one**.
   - Current rule:
     ...
     - introduce a new helper in `async-boundaries.js` only if the site has real semantics that the generic control-flow boundary does not express well
   - Completed in this step:
     - `loop.js` loop `else` body now compiles directly into the current loop buffer with ordinary scope push/pop; it no longer creates a child boundary through `asyncBufferNode(...)`
     ...
     - `loop.js` iteration body wrapper now lowers directly through `runControlFlowBoundary(...)` for async loops and plain scoped emission for sync loops
     - `buffer.js` no longer has any live `asyncBufferNode(...)` call sites, so the helper itself has been removed
   - Result:
...
The original `_compileMacro` experiment produced exactly that failure shape. The final caller-side fix was to give `Caller` bodies their own local `__waited__` chain, so command-only child boundaries contribute a structural completion signal instead of depending on closure counting.
...
**Implemented:** Discriminate on `child._analysis?.mutatedChains?.size > 0`:
...
- **Chain-mutating expressions** still need structural child-buffer lowering, but caller-specific timing must not be handled here.

Important clarification from the waited-loop work:
- `compileOutput` itself does not own a special waited marker
- `compileOutput` must not write to `__caller__`; that chain is owned only by caller invocation code
...
if (child._analysis?.mutatedChains?.size > 0) {
  ...
  this.emit.line(`${this.buffer.currentBuffer}.add(${textCmdExpr}, "${textChainName}");`);
}
```
...
**`_expressionAddsCommands` deleted.** This was the previous ad-hoc AST walk that detected `caller()` and `!` calls. It is now replaced by `child._analysis?.mutatedChains?.size > 0`, which is correct after the analysis fixes below.
...
- `analyzeFunCall` (compiler-base.js): added detection of `caller()` function calls (both direct `caller()` and path-rooted forms). These calls now produce `mutates: [textChain]`, making them visible in `mutatedChains`. Without this, `caller()` calls would fall through to the sync path and crash at runtime.
- `analyzeCallExtension` / `analyzeCallExtensionAsync` (compiler.js): new methods declaring `mutates: [textChain]`. These statement-level extension tags use `asyncAddToBufferScoped` internally but had no analysis.
...
#### `compileChainDeclaration` — `var x = asyncExpr`

**Current:** For `var` chains with an initializer, `asyncAddValueToBuffer` wraps a `VarCommand` construction in an async block.

**New:** Evaluate the initializer expression (result may be a promise), construct and add the `VarCommand` synchronously.

```js
if (chainType === 'var' && node.initializer) {
  const t1 = this._compileExpression(initNode, frame, true, initNode);
  // emit synchronously:
  currentBuffer.add(new VarCommand({ chainName: name, args: [t1], pos }), name);
}
```

---

#### `compileAsyncVarSet` step 3 — var chain assignment

**Current:** Step 3 (emit chain command + export) wraps each `VarCommand` in `asyncAddValueToBuffer`.
...
currentBuffer.add(
  new VarCommand({ chainName: name, args: [valueId], pos: { lineno: node.lineno, colno: node.colno } }),
  name
);
```
...
**New:** Evaluate each expression directly. Results that are promises are fire-and-forget — any side effects they produce go through chains (which handle ordering) or are external (user's responsibility to sequence with `!`). No async block, no `Promise.all` wait.
...
4. Catch block poisons all chains from both branches if condition fails.
...
One simplification over `compileIf`: the old code collected `branchChains` inside each `asyncBlock` callback (after `compile(c.body, f)`). Since `_analysis` is fully populated before compilation starts, `c.body._analysis.usedChains` is readable at any point — `allChains` is now computed directly after the switch block is emitted, without any per-branch collection structure.
...
**Changed in** `loop.js:_compileFor`: outer `asyncBufferNode(node, frame, createScope=true, sequential=false, sourcePositionNode, ...)` replaced by `compileControlFlowBoundary(node, frame, ...)`.
...
**Async nodes**: `compileControlFlowBoundary` already creates a compiler-level `frame.push(false, false)` and sets `this.currentBuffer = 'currentBuffer'`. For async loops (`useLoopValues = true`), loop variables use `VarCommand` chains, not `frame.set()`, so no runtime `frame.push()` is needed in the generated code.
...
- async `ignore missing` composition now returns a real empty command buffer with a `__text__` chain, so include composition can rely on `_setBoundaryAliases()` and direct `SnapshotCommand` enqueueing instead of using defensive duck-typing
...
|---|---|---|
| `compiler.js` | Remove `asyncAddValueToBuffer` from `compileAsyncVarSet` step 3, `compileChainDeclaration`; rewrite `compileDo`; rewrite `compileReturn` (remove `waitAllClosures`) | Rewrite `compileIf`, `compileSwitch` to use `runControlFlowBoundary` |
| `buffer.js` | Remove `asyncAddToBufferScoped` from `compileOutput`; emit `TextCommand` directly | Remove `asyncAddToBuffer`, `asyncAddToBufferScoped` helpers once all call sites gone |
| `loop.js` | — | ? `_compileFor` outer wrapper ? `compileControlFlowBoundary` (item 7); sequential body `waitAllClosures` removal pending (item 8) |
| `inheritance.js` | — | ? Rewrite `compileInclude`, `compileExtends`, `compileImport` |
...
| `async-boundaries.js` | ? Owns `compileControlFlowBoundary`, `compileRenderBoundary`, `compileTextBoundary`, and `compileCaptureBoundary` | Compiler lowering for structural boundaries |
| `buffer.js` | Delegates `compileControlFlowBoundary`; still owns buffer-specific helpers | — |
| `emit.js` | Delegates `compileRenderBoundary`; still owns generic emit utilities such as `getLinkedChainsArg(...)` | — |
...
- **`src/compiler/inheritance.js`**: ? `compileInclude`, `compileExtends`, `compileImport`, and `compileFromImport` migrated to the chains-refactor model.
- **`src/compiler/async-boundaries.js`**: Own compiler lowering for structural boundaries (`compileControlFlowBoundary`, `compileRenderBoundary`, `compileTextBoundary`, `compileCaptureBoundary`).
- **`src/compiler/buffer.js`**: Stop emitting `asyncAddToBuffer` / `asyncAddToBufferScoped`; command-adding paths become synchronous. Keep only the thin `compileControlFlowBoundary` delegate plus buffer-specific helpers.
- **`src/compiler/emit.js`**: Stop emitting `asyncBlockBegin` / `asyncBlockEnd` / `asyncBlockValue` / `asyncBlockRender` from migrated call sites. Keep only the thin `compileRenderBoundary` delegate plus generic emit helpers until Phase 2.
...
- **`src/compiler/emit.js`**: Remove `asyncBlockBegin`, `asyncBlockEnd`, `asyncBlockValue`, `asyncBlockRender`, `asyncBlock`, `getAsyncBlockArgs`.
- **`src/compiler/buffer.js`**: Remove `asyncAddToBuffer`, `asyncAddToBufferScoped`.
- **Generated function signatures**: Remove `astate` parameter and review `cb`.

### Never changes

`CommandBuffer`, `BufferIterator`, `Chain` subclasses, all command classes, `CompileAnalysis`, `CompileAsync`, `sequential.js`, `loop.js` (structure only), the poison system, and `resolveCommandArgumentsForApply`.
