# `extends` Implementation 3

This document is a working design note for a more radical simplification of the
current `extends` / inheritance / component implementation.

It starts from the proposal below and will evolve from there.

## Proposal

This is the proposed direction, with only minimal edits for clarity.

1. Have an easy way to detect block/method scripts/templates.
2. Move the blocks/methods to a separate AST node in the transformer.
3. Compile this node into a key-value object (`methods`) at the start of the
   JS file. The keys are the method/block names. The values contain both
   metadata (like used/mutated channels), the function, as well as the `super`
   object. The constructor is part of this too, named `__constructor__` or
   similar.
4. Additionally, collect all parent methods: methods that are either not yet
   defined, or used through `super()`. These are compiled into the
   block/methods object as promises, with `resolve` and `reject` methods, so 3
   properties: `promise`, `resolve`, `reject`. Once a promise is resolved by
   the parent, it will be replaced with the value object (metadata and
   function). The `super` properties are also a promise in the same way. If we
   do not have a definition for a called method, we add it to the methods
   object, not as `super`.
5. Also collect all shared channels. The key is the channel name. The value is
   shared-channel metadata including at least the channel type and the local
   default value. Unknown shared channels use the same promise structure and
   are replaced in place once resolved.
6. A parent script/template is called with one shared metadata object,
   containing at least:
   - `methods`
   - `sharedRootBuffer`
   - `sharedSchema`
   - `compositionPayload`
7. The parent script will resolve all method object / `super` promises that it
   can, and will replace the target with the resolved object. It will add its
   own methods to that object, for methods that are not already there, and pass
   it to the parent if `extends` is used. If no `extends` is used and there are
   still promise methods that have not been resolved, these will be rejected.
8. The method calls will reference the method from the method object, which can
   be either the promise structure or a resolved object.
9. The side-channel apply, when calling a promise method, must first await it.
10. Each script/template adds shared channels to the shared schema when they do
    not exist yet, resolves pending entries when it can, and rejects unresolved
    ones when it is the root.
11. Do not read used/mutated channels directly from the raw method metadata
    object. Instead, use a helper that:
...
13. Shared channel metadata should follow the same model as method metadata.
    ...
    channel type and the local default value. A not-yet-known channel is stored
    as a promise structure with `promise`, `resolve`, and `reject`, and is
    replaced with the resolved metadata once a parent defines it.
14. Shared channel access should use a dedicated helper, analogous to the
    method helper. It:
...
15. The promise returned by the shared-channel helper is the barrier for
...
- Per-template state is the compiled JS shape. Per-instance state is that same
  ...
  chain.
- The design assumes a static parent chain. Parent paths are declared
  statically in source and are not computed dynamically.
...
- For scripts/templates that define blocks or methods, the first thing the
  script does at startup is wire its local method metadata into the shared
  method object. That is where child-overrides-parent semantics are
  established.
...
- `extends` creates an async boundary. Code after `extends` executes in a
  later async boundary / command buffer, similar in spirit to how other
  structural async boundaries split execution.
- Startup register / resolve / reject is synchronous. It only updates the
  shared metadata object using already-available local metadata:
  - declare if missing
  - resolve if the current entry is a promise structure
  - wire `super` if a child method uses `super()` and the local parent method
    matches
- Each script/template tries to resolve pending methods, pending shared
  channels, and pending method `super` entries using its own local definitions.
  When the root script/template is reached, any still-pending entries are
  rejected there.
- Unresolved vs resolved entries should keep the smallest possible stable
  shape:
...
- The shared channel schema should stay explicit: key is identifier, value is
  shared-channel metadata containing at least the channel type and local
  default value. This is still needed so each script/template can add channels
  that are not yet present in the shared command buffer.
- `shared` means hierarchy-owned instance state. `extern` remains the
...
- Re-declaring an existing shared channel with a different type is a
  ...
  applies.
- The resolved shared-channel metadata shape should stay small, but it now
  needs more than just the type:
  - channel type
  - local default value
- That is enough to choose the side-channel command shape, detect conflicting
  ...
  defaults.
- Parent invocation should be normalized into one object-shaped argument,
  containing at least:
  - `methods`
  - `sharedRootBuffer`
  - `sharedSchema`
  - `compositionPayload`
- `methods` and `sharedSchema` are the shared key/value objects passed up the
  extends chain. Pending promise structures are added to those shared objects,
  and are later replaced in place with the resolved values.
- The initial entrypoint is the import-run script/template itself. It executes
  ...
  the shared metadata object. Imported components should not have a meaningful
  return value of their own. Component constructor return is ignored so one
  script can still serve both as a component and as an extended script. In all
  cases, the runtime value that matters is the shared metadata object.
...
- The helper described in points 11-12 is the exact-link-after-load mechanism.
  It is awaited by `apply()` before linking, and it does not resolve until the
...
- Entry replacement belongs to startup registration, not to the helper. The
  helper may still memoize merged metadata on a resolved entry so repeated
  calls do not redo the same merge work.
- When apply requests metadata through the helper, it awaits the already-shared
  ...
  already happened. The helper only computes and memoizes derived merged
  metadata such as effective channel sets.
- When a child defines the same method name as its parent, child overrides
  ...
  should create a pending `super` entry in the child's `super` property. The
  parent resolves that pending entry during its own startup wiring, so the
  later helper resolution lands on the parent's final resolved method object.
- The default error model should be strict:
  ...
  - missing `__constructor__` is the one allowed non-error case and should map
    to an empty constructor method
- Topmost-parent rejection becomes observable when apply awaits the helper. If
  ...
  `RuntimeFatalError`.
- Components should use the same shared metadata object model. On the caller
  side, apply should:
...
- Shared channel lookup from the caller script/template should mirror method
  lookup. It should use a dedicated helper that:
  ...
  - throws if the topmost parent is reached and the channel still does not
    exist
- That shared-channel helper should use the same pending/resolved/rejected
  model as method entries. The helper result is the effective shared-channel
  metadata used to choose and construct the side-channel command. Allowed
  shared-channel operations should stay explicit:
  - `snapshot()`
  - `isError()`
  - `getError()`
...
- The main body is the constructor. If the constructor is empty, `super()` is
  implicit, except at the root level where an empty constructor stays empty.
  This means every non-root level effectively has a constructor. If the
  constructor is not empty, parent-constructor execution is not automatic; it
  happens only through the constructor's own `super()` behavior.
...
- The minimum resolved method-entry shape should stay small:
  ...
  - compiled function
  - `super`
- The compiler should emit only the channels directly touched by the local
  method body. Channels coming from the `super` chain are unknown at compile
  time and are merged later by the helper.
- Keep `usedChannels` and `mutatedChannels` separate in the stored method
  entry, but merge conservatively when needed. Treat `usedChannels` as
  read+write for safety if existing compiler behavior is inconsistent.
- `super` should stay part of the resolved method metadata shape rather than
  becoming a separate lookup table. A `super()` call should resolve that
  `super` property through the same helper model, or through a dedicated
  helper that behaves the same way.
...
- Call sites should not read `usedChannels` / `mutatedChannels` directly from
  the raw stored entry. They should go through the helper, which resolves the
...
- A separate dynamic-extends adapter is not an architectural requirement if the
  ...
  still exist as an implementation seam for code organization, but it should
  not introduce a second resolution model.
- Each component instance should get its own shared metadata object. More
  generally, each extends-linked chain, whether run directly or imported as a
  component, gets its own shared metadata object.
- Component use should have explicit syntax. Use a `component` keyword instead
...
- If a script/template receives the shared metadata object but defines no
  blocks/methods, it should still be able to declare or override the
  constructor. The compiled code for that case should stay minimal.
...
- Regular import and component import should remain distinct:
...
- Return semantics stay explicit:
  ...
  - for component instantiation, constructor return is ignored and the
    component object is produced
...
- Multiple component instantiations are fully independent instances with
  separate shared roots, shared state, side-channels, and method calls.
...
- Open questions, missing pieces, and compatibility constraints will be added
  in later revisions instead of being front-loaded here.
