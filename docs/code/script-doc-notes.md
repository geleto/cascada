...
+ "### Error handling and recovery with macros - When an Error Value is passed as an argument to a macro, the macro body is skipped entirely and the macro immediately returns a poisoned value." - This is not true anymore, if arguments are poisoned, the macro gets them as arguments and can handle the poisoned values
