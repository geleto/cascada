...
The compiler currently keeps scope declarations in `declaredChannels`. That map
...
same declaration rules as the current analysis `declaredChannels` machinery:

- declaration kinds: `var`, `data`, `text`, `sequence`, imports, callable
  arguments, macro params, root return/text channels, shared declarations, and
...
`declaredChannels` can remain the inventory if useful, but symbol resolution
must use source-order lookup.

## Callable Arguments

Function, macro, inherited-method, block, and constructor arguments are local
`var` channels. They are different from source-order body declarations because
...
Default argument expressions should also see all argument channels declared for
the callable frame. This is why compiler emission declares all argument local
channels before it emits any argument initialization commands.
...
1. Ask the source-order declaration log for the declaration visible at this
   source point.
2. If found, compile/analyze it as a local channel/var lookup.
...
- Conflict checks can still happen when each declaration is reached. For
  ...
  can fail at `var x`.
- Analysis still needs aggregate facts such as `usedChannels`, `mutatedChannels`,
  linked channels, and callable footprints.
...
- `print(someVar); var someVar = 5` in script mode compiles the first
  `someVar` as ambient, not as a local channel read.
...
- Default argument expressions can read earlier and later argument channels
