...
- [Error Handling: ErrorCommand vs PoisonedValue](#error-handling-errorcommand-vs-poisonedvalue)
...
## Error Handling: ErrorCommand vs PoisonedValue
...
### Template mode: PoisonedValue

Template mode (and guard recovery scanning) uses `PoisonedValue` directly in buffers:

```javascript
array.push(new PoisonedValue(processedErrors));
```

These are detected by `isPoison(item)` checks in both `processItem` and
...
| Poison entries | `PoisonedValue` instances | `ErrorCommand` instances |
...
- replace any compiler sites that write poison entries or buffer slots with output-owned logic
