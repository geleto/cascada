...
- when async mode uses plain JS locals instead of chains, how do those values
...
- chain state participates naturally in buffer/chain ancestry
- JS locals do not
...
- shared mutable state: use chains/outputs instead
...
- the modern async loop path already uses chains for loop variables and loop metadata
...
- keep it read-only
- use chains for shared mutable state
...
front if the relevant async state remains chain-backed.
