...
2. Runs `node scripts/runprecompile.js` with `CASCADA_TEST_DIST=1`, so the browser fixture is generated by `dist/precompile.js`.
3. Runs the browser test runner with `/src/...` module requests served from `dist/...`.
4. Runs the Node test runner with a test-only ESM loader that redirects `src` imports to matching `dist` files.
...
- Source lanes collect coverage from `src/**/*.js`; `npm test` collects coverage from `dist/**/*.js`.
