...
*   `new AsyncEnvironment([loaders], [opts])`
    ...
    *   `opts`: An object with configuration flags:
        *   `autoescape` (default: `true`): If `true`, automatically escapes output from templates to prevent XSS attacks. This does not affect the data generated by scripts.
...
For maximum performance, you should precompile scripts and templates to JavaScript ahead of time, which eliminates all parsing overhead at runtime.
...
These functions return a string of JavaScript code. You can save this code to a `.js` file and use a `PrecompiledLoader` to load your assets instantly. See the Nunjucks documentation on [Precompiling](https://mozilla.github.io/nunjucks/api.html#precompiling) for a detailed guide.