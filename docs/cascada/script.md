...
// @set: Sets or creates a value at a path
@set user.id userId
@set user.name userProfile.name
...
@set user.settings.theme "dark" // Overwrite one setting
```
</details>
</td>
<td width="50%" valign="top">
...
*   **`data`**: A structured object or array built with commands like `@set`, `@push`, and `@merge`. This is typically the main output of a script.
*   **`text`**: A simple string of text generated by `@text` commands.
...
#### Built-in Data Commands (`@set`, `@push`, etc.)
...
| Command | Description |
|---|---|
| `@set path value` | **Replaces** the value at `path`. |
...
##### The `@text` Command
The `@text` command is a versatile tool for generating text output. Its behavior depends on whether a path is provided.

*   **`@text path value`**: **Appends** the `value` to the content at `path` in the `data` object. If the existing content is a string, it concatenates.
*   **`@text value`** (no path): Appends to the global text output stream, which populates the `text` property of the result object.
...
macro buildDepartment(deptId) : data
  ...
  @set department.manager manager.name
  @set department.teamSize team.length
endmacro
...
@set company.sales salesDept.department
```
</details>
</td>
<td width="50%" valign="top">
...
@set company.projectA projectReport.report
```
</details>
</td>
<td width="50%" valign="top">
